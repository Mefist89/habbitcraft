    ...
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-br from-bg-start to-bg-end px-6 py-8">
      ...
      <main className="flex-1 w-full max-w-md flex flex-col items-center justify-center text-center mt-[-20px]">
        ...
        <p className="font-body text-sm text-on-surface-variant leading-relaxed mb-10 px-4">
          Log in safely to continue tracking progress and unlocking new worlds!
        </p>
        ...
        <div className="w-full space-y-4 mb-8">
          <button
            ...
            disabled={isLoading}
            className="w-full relative flex items-center justify-center gap-3 py-3.5 px-4 bg-white text-on-surface font-headline font-bold text-sm rounded-full border-2 border-outline hover:bg-black/5 hover:border-black/20 transition-all active:scale-95 disabled:opacity-70 disabled:pointer-events-none group"
          >
            {isLoading ? (
      ...
      <p className="text-center font-body text-[11px] text-on-surface-variant/40 mt-8">
        HabbitCraft uses secure parent-approved login.
      </p>
    </div>
  );
}
